...
int choice_your_function()
{
    int vibor = -1;
    printgran(53);
    printf(" Вам доступен набор, состоящий из следующих функций:\n (1) sin(x),%5cx вводится в градусах\n (2) cos(x),%5cx вводится в градусах\n (3) arcsin(x), |x|<1\n (4) arccos(x), |x|<1\n (5) sinh(x)\n (6) cosh(x)\n (7) exp(x)\n", ' ', ' ');
    printgran(53);
    printf(" Введите номер функции, с который Вы хотите работать:\n");
...
double choice_your_dot(int vibor)
{
    ...
    while (prov != 2)
    {
        printf("\n Задайте точку x, в которой необходимо вычислить значение:\n");
        while (prov != 1)
        {
            prov = scanf_s("%lf", &x);
            if (prov != 1)
        ...
        if (((vibor == 3) || (vibor == 4)) && (fabs(x) >= 1))
        {
            printf(" Введено недопустимое значение x. Функции arcsin(x) и arccos(x) определны при |x| < 1. Попробуйте ещё раз.\n");
            while (getchar() != '\n');
            prov = 0;
        }
        else
        ...
        if ((vibor == 1) || (vibor == 2))
        {
            printf(" Происходит преобразование значения переменной x (из градусов в rad).\n Если Вы видите данное сообщение, пожалуйста, подождите.\n");
            ...
            while (fabs(x) > 2 * PI) //Чтобы убрать ущерб, доставляемый большими значениями x
            {
                x = x > 0 ? x - 2 * PI : x + 2 * PI;
            }
            system("cls");
...
void navigator(int vibormenu)
{
    ...
    case 1:
        ...
        printf("\n\n Задайте точность вычисления eps (>= 0,000001):\n");
        while (eps < 0.000001)
        {
            scanf_s("%lf", &eps);
            if (eps < 0.000001)
