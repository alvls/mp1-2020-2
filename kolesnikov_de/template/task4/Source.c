...
int pricelist[] = { 1543,1654,7654,2434,5453,434,545,6325,9876 };
char description[] = ("Smart Digital Picture Frame 10.1Multipurpose Foldable Computer Study Desk - Black"
	...
	"AmazonBasics Replacement Water Filters for Water Pitchers"
...
void FinalSum(int mode) {
	for (k,i = 0; i < NumOfProducts,k<countproducts; i++,k++) {
		if (barcodeout[i] == 1) {
			temp1 = pricelist[i];
			temp2 = productcount[i];
			sum += temp1*temp2;
			sumd += temp1*(discountlist[i]/100);
			totalsum += (sum - sumd/100)*temp2;
		}
	}
	if (mode == 1) {
		printf("Sum without diskount`s = %d, sum of discounts = %d, total sum = %d", sum, sumd, totalsum);
	}
	else
...
void makeout() {
	for (k=0,i = 0; i < NumOfProducts,k<countproducts; i++,k++) {
		if (barcodeout[i] == 1){
