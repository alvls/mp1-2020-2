...
void main()
{
	...
	double p_dvp = 0.00082; //кг/см^3  0.00082
	double p_dsp = 0.00072; //кг/см^3 0.00072
	double p_wood = 0.00081; //кг/см^3 0.00081
	double h, w, t1 = 0.5; //Задняя стенка(ДВП) h = 180-220cm w = 80 - 120cm t1(толщина задней стенки)
	double d, t2 = 1.5, t3 = 1;//Боковины(ДСП) h =180-220 d = 50-90(глубина) t2(толщина боковин)
	...
	if ((h <= 220) && (h >= 180) && (w >= 80) && (w <= 120))
	{
		...
		if ((d >= 50) && (d <= 90))
		{
			...
			m5 = p_dsp * quantity * (t2 * (w - 2 * t2) * (d - t1));
